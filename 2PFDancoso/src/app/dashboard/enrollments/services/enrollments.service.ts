...
export class EnrollmentsService {
  ...
  addEnrollment(enrollment: Enrollment){
    this.enrollmentsList.push({...enrollment, id: new Date().getTime()});
  ...
  updateEnrollment(enrollmentToUpdate: Enrollment){
    let enrollment = this.enrollmentsList.find(s=> s.id === enrollmentToUpdate.id);
    
    const enrollmentIndex = this.enrollmentsList.findIndex((s=> s.id === enrollmentToUpdate.id));
    if(enrollmentIndex != -1){
      this.enrollmentsList[enrollmentIndex] = { ...enrollment, ...enrollmentToUpdate};
      this.enrollments.next(this.enrollmentsList);
      this.enrollmentsUpdated.emit();
    }
  }  
